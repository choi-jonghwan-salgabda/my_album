...
try:
    ...
    from my_utils.object_utils.photo_utils import rotate_image_if_needed, compute_sha256, _get_string_key_from_config
    from my_utils.object_utils.JsonManager import JsonManager # JsonConfigHandler 임포트
    from my_utils.object_utils.yolo_utils import YoloDetector
    from my_utils.object_utils.dlib_utils import DlibFaceProcessor
...
DEFAULT_STATUS_TEMPLATE  = {
    ...
    "error_get_target_file":        {"value": 0,  "msg": "처리대상(image or json) 파일 읽기 오류 수"},
    "error_exist_target_file":        {"value": 0,  "msg": "처리대상(image or json) 파일이 없어요"},
    "error_rotation_image":          {"value": 0,  "msg": "이미지 회전중 오류 발생 파일 수"},
    "error_read_input_files":          {"value": 0,  "msg": "읽은 입력 파일 수 (detect_object 기준)"},
    "total_object_count":      {"value": 0,  "msg": "검출된 총 객체 수"},
    "get_object_crop":           {"value": 0,  "msg": "객체가 검출된 객체 수"},
    "error_object_bbox_cnt":           {"value": 0,  "msg": "객체 BBOX 개수 오류 수"},
...
def detect_faces_in_crop_yolo_internal(
    json_handler: JsonManager, # JsonManager 인스턴스 사용
    image_crop: cv2.Mat,
    obj_bbox_origin_xyxy: List[int],
    yolo_detector: YoloDetector,
    dlib_processor: DlibFaceProcessor,
    image_identifier: str,  # 이미지 식별자 (예: 파일명)
    obj_idx: int,           # 현재 처리 중인 객체의 인덱스
    save_debug_crop_dir: Path # 디버그용 크롭 이미지 저장 경로
) -> List[Dict]:
    """
    YOLO 얼굴 탐지 결과를 반환합니다. JsonManager를 사용하여 JSON 키 이름을 참조합니다.
    크롭된 이미지 영역에서 YOLO 모델을 사용하여 얼굴을 탐지합니다.
    탐지된 얼굴의 경계 상자는 원본 이미지 좌표 기준으로 반환됩니다.
    yolo_detector는 이미 적절한 장치(CPU/GPU)에 로드되어 있어야 합니다.
    Args:
        json_handler (JsonManager): JSON 키 설정을 관리하는 핸들러입니다.
        image_crop (cv2.Mat): 얼굴 탐지를 수행할 잘라낸 객체 이미지입니다.
        obj_bbox_origin_xyxy (List[int]): 잘라낸 객체의 원본 이미지 기준 [x1, y1, x2, y2] 좌표입니다.
        yolo_detector (YoloDetector): 미리 로드된 YOLO 얼굴 탐지기 클래스 인스턴스.
        dlib_processor (DlibFaceProcessor): 미리 로드된 dlib 얼굴 특징 추출기 클래스 인스턴스.
    ...
    results = yolo_detector.detect(image_crop)
    logger.debug(f"[{image_identifier}] YOLO 얼굴 탐지 결과 (객체 ID {obj_idx}): {results}")
        ...
        for i, box in enumerate(r.boxes): # 더 자세한 로깅을 위해 enumerate 사용
            ...
            if face_x1_clamped < face_x2_clamped and face_y1_clamped < face_y2_clamped:
                ...
                if face_image_for_embedding.size > 0:
                    # DlibFaceProcessor를 사용하여 임베딩 추출
                    embedding_np = dlib_processor.get_embedding_from_image_crop(face_image_for_embedding)

                    if embedding_np is not None:
                        actual_embedding = embedding_np.tolist()  # numpy 배열을 리스트로 변환
...
def detect_face(
    ...
    yolo_detector: YoloDetector, # YoloDetector 클래스 인스턴스
    dlib_processor: DlibFaceProcessor # DlibFaceProcessor 클래스 인스턴스
    ):
    # 이미지 파일 읽기 (OpenCV 사용)
    ...
    # JsonManager의 read_json 메소드 사용
    parsed_json_data = json_handler.read_json(json_path=input_path)
    ...
    json_image_hash_val     = json_image_info_data.get(json_handler.image_hash_key)
    
    # 해상도 정보는 image_info_group 아래 resolution_key 그룹 아래에 있음
    json_resolution_data = json_image_info_data.get(json_handler.image_resolution_key, {})
    json_width_val       = json_resolution_data.get(json_handler.image_width_key, None)
    json_height_val      = json_resolution_data.get(json_handler.image_height_key, None)
    json_channels_val    = json_resolution_data.get(json_handler.image_channels_key, None)
    ...
    if not json_image_path.is_file():
        ...
        status["error_exist_target_file"]["value"] += 1
        add_path_to_list_file(input_path, undetect_list_path)
        return status
    ...
    if rotate_image_if_needed(str(json_image_path)) == False:
        ...
        status["error_rotation_image"]["value"] += 1
        return status

    # 4.1. 이미지 파일 읽기
    actual_image = cv2.imread(str(json_image_path))
    if actual_image is None:
        status["error_read_input_files"]["value"] += 1
    ...
    for obj_idx, obj_dict_data in enumerate(detected_objects_list):
        ...
        obj_bbox_xyxy = obj_dict_data.get(json_handler.object_box_xyxy_key)

        if not obj_bbox_xyxy or len(obj_bbox_xyxy) != 4:
            logger.warning(f"파일: {input_path.name}, 객체 ID: {obj_idx:3}의 bbox 좌표({json_handler.object_box_xyxy_key})가 없거나 길이가 4가 아님: {obj_bbox_xyxy}")
        ...
        if object_crop_image.size > 0: # 크롭된 이미지가 비어있지 않은 경우에만 탐지 시도
            try:
                ...
                faces_in_object = detect_faces_in_crop_yolo_internal(
                    json_handler = json_handler, # JsonManager 인스턴스 전달
                    image_crop=object_crop_image,
                    obj_bbox_origin_xyxy=[x1_c, y1_c, x2_c, y2_c],
                    yolo_detector=yolo_detector,
                    dlib_processor=dlib_processor,
        ...
        if faces_in_object:
            ...
            # JsonManager의 face_info_key (예: "detected_face")를 사용하여 얼굴 정보 추가.
    ...
    if not final_image_hash:
        ...
        if actual_image is not None: # actual_image 사용
            final_image_hash = compute_sha256(actual_image)
        else: # 이미지가 로드되지 않은 극단적인 경우
            final_image_hash = "N/A"

    try:
        # JsonManager의 write_json 메소드 사용
        write_successful = json_handler.write_json(
        ...
        if write_successful:
            status["files_json_update"]["value"] += 1 # 새 키 사용
            logger.info(f"업데이트된 JSON 파일 저장 성공 (덮어쓰기): {input_path}")
        else:
            # json_handler.write_json이 False를 반환한 경우 (내부적으로 오류 처리)
            status["error_json_update"]["value"] += 1
            logger.error(f"json_handler.write_json을 사용하여 업데이트된 JSON 파일 저장 실패 (반환값 False): {input_path}")
    except Exception as e_save_json:
        status["error_json_update"]["value"] += 1
        logger.error(f"json_handler.write_json을 사용하여 업데이트된 JSON 파일 저장 중 예외 발생: {input_path}, 오류: {e_save_json}", exc_info=True)
...
def run_main(cfg: configger):
    ...
    # 1. 모델 클래스 인스턴스 생성
    # 1.1. YOLO 얼굴 탐지기
    yolo_config_key = 'models.object_yolo_tiny_model.face_detection_model'
    yolo_face_detector = YoloDetector(cfg, model_config_key=yolo_config_key)
    if not yolo_face_detector.is_ready:
        logger.critical("YOLO 얼굴 탐지기 초기화에 실패하여 프로세스를 중단합니다.")
        sys.exit(1)

    # 1.2. Dlib 얼굴 특징 추출기
    dlib_face_processor = DlibFaceProcessor(cfg)
    if not dlib_face_processor.is_ready:
        logger.critical("Dlib 얼굴 특징 추출기 초기화에 실패하여 프로세스를 중단합니다.")
        sys.exit(1)
    ...
    try:
        ...
        # JsonManager 인스턴스 생성
        try:
            json_handler = JsonManager(json_cfg)
