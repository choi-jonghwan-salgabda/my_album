...
class OCRPLModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for key, value in pred['loss_dict'].items():
            ...
            if isinstance(self.logger, pl.loggers.WandbLogger):
                self.logger.experiment.summary[f"train/{key}"] = value
        return pred
    ...
    def on_validation_epoch_end(self):
        ...
        if isinstance(self.logger, pl.loggers.WandbLogger):
            self.logger.experiment.summary["val/recall"] = recall
            self.logger.experiment.summary["val/precision"] = precision
            self.logger.experiment.summary["val/hmean"] = hmean
    ...
    def on_test_epoch_end(self):
        ...
        if isinstance(self.logger, pl.loggers.WandbLogger):
            self.logger.experiment.summary["test/recall"] = recall
            self.logger.experiment.summary["test/precision"] = precision
            self.logger.experiment.summary["test/hmean"] = hmean
    ...
    def on_predict_epoch_end(self):
        ...
        if isinstance(self.logger, pl.loggers.WandbLogger):
            table = self.logger.experiment.Table(columns=["Image", "Filename", "Num Boxes"])

            for filename, pred_boxes in self.predict_step_outputs.items():
                img_path = str(Path(self.config.datasets.predict_dataset.image_path) / filename)

                wandb_img = self.logger.experiment.Image(img_path)
                table.add_data(wandb_img, filename, len(pred_boxes))

            self.logger.experiment.log({"Predictions": table})
