...
class OCRDataset(Dataset):
    def __init__(self, image_path, annotation_path, transform):
        ...
        self.valid_images = []  # 유효한 이미지 목록 추가
                    ...
                    if file.suffix.lower() == f'.{ext}':
                        try:
                            with Image.open(file) as img:
                                ...
                                self.valid_images.append(file.name)
                        except (IOError, PIL.UnidentifiedImageError, OSError):
                            print(f"손상된 파일 발견: {file} - 데이터셋에서 제외됨")
                            continue
            return
            ...
            for filename in annotations['images'].keys():
                ...
                if image_file.exists():
                    try:
                        ...
                        print(f"손상된 파일 발견: {image_file} - 데이터셋에서 제외됨")
                        continue
        
        print(f"유효한 이미지 수: {len(self.valid_images)}/{len(annotations['images'])} (제외된 이미지: {len(annotations['images']) - len(self.valid_images)})")

    def __len__(self):
        return len(self.valid_images)

    def __getitem__(self, idx):
        image_filename = self.valid_images[idx]
