...
if __name__ == "__main__":
    ...
    args_for_log = {k: str(v) if isinstance(v, Path) else v for k, v in vars(parsed_args).items()}
    # 인자 딕셔너리를 pprint.pformat을 사용하여 보기 좋은 문자열로 변환
    formatted_args = pprint.pformat(args_for_log, indent=4, width=100)
    logger.info(f"명령줄 인자:\n{formatted_args}")
