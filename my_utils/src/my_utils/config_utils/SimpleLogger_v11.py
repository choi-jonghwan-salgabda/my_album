...
class SimpleLogger:
    ...
    def setup(self, logger_path=None, include_function_name=None, pretty_print=None, async_file_writing=None, console_min_level='WARNING', file_min_level=None):
        """
...
class DiskSpaceMonitor:
    """
    디스크 공간을 모니터링하고 사용량이 높을 때 사전에 쓰기 오류를 방지하는 싱글톤 클래스입니다.
    """
    _instance = None
    ...
    def __init__(self, threshold_percent=80.0, check_interval_secs=10):
        """
        모니터를 초기화합니다. 싱글톤 패턴으로 인해 이 초기화는 한 번만 실행됩니다.
        Args:
            threshold_percent (float): 정밀 모니터링을 시작할 디스크 사용량 임계값 (%).
            check_interval_secs (int): 임계값 미만일 때 사용량을 다시 확인할 간격 (초).
        """
        ...
        with self._lock:
            ...
            self._monitoring_devices = {}  # {dev_id: {'monitoring': bool, 'last_check': float}}
            self._initialized = True
            logger.debug(f"DiskSpaceMonitor 초기화 완료: 임계값={self._threshold}%, 확인 간격={self._interval}초.")

    def _get_device_info(self, path: Path) -> Tuple[int, Path]:
        ...
        try:
            ...
            logger.error(f"'{path}' 경로의 장치 정보를 가져올 수 없습니다: {e}")
            root_path = Path('/')
            return os.stat(root_path).st_dev, root_path

    def _update_monitoring_status(self, dev_id: int, existing_path_on_device: Path):
        """주기적으로 디스크 사용량을 확인하고 모니터링 상태를 업데이트합니다."""
        now = time.time()
        dev_info = self._monitoring_devices.get(dev_id, {'monitoring': False, 'last_check': 0})

        if not dev_info['monitoring'] and (now - dev_info['last_check'] > self._interval):
            try:
                usage = shutil.disk_usage(existing_path_on_device)
                percent_used = (usage.used / usage.total) * 100
                dev_info['last_check'] = now
                if percent_used >= self._threshold:
                    logger.warning(f"장치 {dev_id}의 디스크 사용량이 {percent_used:.2f}%로, 임계값({self._threshold}%)을 초과했습니다. 이 장치에 대한 정밀 검사를 활성화합니다.")
                    dev_info['monitoring'] = True
                self._monitoring_devices[dev_id] = dev_info
            except Exception as e:
                logger.error(f"장치 {dev_id}의 디스크 공간 확인 실패: {e}")
                self._monitoring_devices[dev_id] = {'monitoring': False, 'last_check': now}

    def pre_write_check(self, dest_path: Path, size_to_write: int):
        """파일을 쓰기 전에 디스크 공간이 충분한지 확인합니다."""
        with self._lock: # Ensure thread safety for monitor state
            dev_id, existing_path_on_device = self._get_device_info(dest_path)
            self._update_monitoring_status(dev_id, existing_path_on_device)
            dev_info = self._monitoring_devices.get(dev_id) # Retrieve updated info
            if dev_info and dev_info['monitoring']: # Only check if monitoring is active for this device
                try:
                    usage = shutil.disk_usage(existing_path_on_device)
                    ...
                    logger.error(f"디스크 공간 사전 검사 중 오류 발생: {e}")
...
def safe_move(src: str, dst: str):
    ...
    try:
        ...
        dst_dev, _ = disk_monitor._get_device_info(dst_path) # Only need dev_id here
        if src_dev != dst_dev:
            file_size = src_path.stat().st_size
            disk_monitor.pre_write_check(dst_path, file_size)
        shutil.move(src, dst)
...
if __name__ == "__main__":
   ...
   # 1. 명령줄 인자 파싱
    ...
    def test_disk_space_monitor():
        ...
        temp_dir = None
        original_threshold = disk_monitor._threshold # Store original threshold
        original_interval = disk_monitor._interval # Store original interval
        ...
        try:
            ...
            disk_monitor._threshold = 0.0001
            disk_monitor._interval = 0.01 # 짧은 간격으로 확인하여 즉시 모니터링 상태 반영
            disk_monitor._monitoring_devices = {} # 이전 모니터링 상태 초기화
            ...
            disk_monitor._threshold = original_threshold
            disk_monitor._interval = original_interval
            disk_monitor._monitoring_devices = {} # 모니터링 상태 초기화
