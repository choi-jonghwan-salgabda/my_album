...
class JsonConfigHandler:
    ...
    def __init__(self, json_keys_config: Dict[str, Any]):
        ...
        """
        logger.debug(f"JsonConfigHandler 초기화 시작: root_dir='{json_keys_config}'")
...
if __name__ == "__main__":
   ...
   # 1. 명령줄 인자 파싱
    ...
    logger.console(f"--- configger.py test execution ---")
    ...
    # 4. JsonManager 초기화
    logger.console(f"--- JsonManager.py test execution ---")
    try:
        json_keys_config = config_manager.get_config("json_keys")
        logger.debug(f"json_keys_config :{json_keys_config}")
        if json_keys_config:
            ...
            logger.debug(f"새 JsonManager 인스턴스 생성 완료.")
            ...
            if success_new:
                logger.debug(f"[성공] 새 JSON 파일이 저장되었습니다: {test_json_path_new}")
                logger.debug(json.dumps(json_manager_new.dump(), ensure_ascii=False, indent=2))
            ...
            if existing_json_path.exists():
                ...
                logger.debug(f"[정보] 기존 JSON 로드 완료: {existing_json_path}")

                # 데이터 수정 (set 메소드 사용)
                logger.debug("[정보] 이미지 이름과 사용자 이메일 수정 중...")
                ...
                if success_modified:
                    logger.debug(f"[성공] 수정된 JSON 파일이 저장되었습니다: {test_json_path_modified}")
                    logger.debug(json.dumps(json_manager_existing.dump(), ensure_ascii=False, indent=2))
            else:
