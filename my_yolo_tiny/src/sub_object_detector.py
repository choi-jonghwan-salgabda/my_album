"""
이 스크립트는 이미지 내에서 이전에 식별된 객체에 대해 얼굴 감지를 수행합니다.

객체 감지 결과가 포함된 JSON 파일을 읽고, 해당 이미지에서 각 객체를 잘라낸 다음,
잘라낸 객체에 대해 YOLO 기반 얼굴 감지 모델을 실행합니다. 얼굴이 감지되면
경계 상자와 `face_recognition` 라이브러리를 사용한 얼굴 임베딩을 추출합니다.

그런 다음 원본 JSON 파일은 각 객체에 대한 이 새로운 얼굴 정보로 업데이트됩니다.
이 스크립트는 초기 객체 감지 단계(예: `object_detector.py`) 이후에 실행되도록
설계되었으며, 특정 얼굴 세부 정보로 데이터를 보강하여 후속 분석이나
얼굴 인식 작업에 사용될 수 있도록 합니다.
"""
import json
import os
...
def detect_faces_in_crop_yolo_internal(
    json_handler: JsonConfigHandler, # JsonConfigHandler 인스턴스 사용
    image_crop: cv2.Mat,
...
def detect_face(
    ...
    # JsonConfigHandler의 read_json 메소드 사용
    ...
    try:
        # JsonConfigHandler의 write_json 메소드 사용
    ...
    for input_path in input_file_iterator_for_processing: # 이터레이터를 순회
        if input_path.is_file(): # 파일인지 다시 확인 (glob 결과는 대부분 파일이지만 안전을 위해)
            ...
            try:
                ...
                """
                ret = detect_face( # JsonConfigHandler 인스턴스 전달
